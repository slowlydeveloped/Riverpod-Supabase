...
class AdminFunctions extends StateNotifier<void> {
  ...
  Future<List<OrderItemModel>> fetchOrderItems() async {
    try {
      final orders = await supabaseClient.from('order_items').select('*, recipe(recipe_name)');
      // Mapping the response data to OrderItemModel objects and returning as a list
      return orders
          .map<OrderItemModel>((data) => OrderItemModel.fromJson(data).copyWith(
...
final recipeCreationProvider = StateNotifierProvider<AdminFunctions, void>((ref) {
  final supabaseClient = ref.watch(supabaseClientProvider);
  return AdminFunctions(supabaseClient);
});
